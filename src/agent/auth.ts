...
export async function issueIdentityAccessKey(identity: AgentIdentity, ttlMs = 30 * 24 * 60 * 60_000) {
  await assertIdentityActive(identity.id);
  const token = secret("ah2_access");
  const expiresAt = Date.now() + Math.min(Math.max(ttlMs, 60_000), 30 * 24 * 60 * 60_000);
  const id = `key_${randomBytes(16).toString("hex")}`;
  await insertAccessKey({ id, identityId: identity.id, tokenHash: hash(token), expiresAt });
  return { id, token, expiresAt };
}

export async function authenticateIdentityAccessKey(token: string) {
  const identity = await findIdentityByAccessHash(hash(token));
  if (!identity) throw new Error("Invalid identity access key");
  return identity;
}
...
export async function authenticateAgent(token: string) {
  const credential = await findCredentialByHash(hash(token));
  if (!credential) throw new Error("Invalid agent credential");
  return credential;
}
