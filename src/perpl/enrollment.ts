...
export async function requestEnrollmentPayload(params: {
  ...
  if (!response.ok) throw new Error(`Perpl enrollment payload ${response.status}: ${await response.text()}`);
...
export async function enrollApiKey(params: {
  ...
  if (!response.ok) throw new Error(`Perpl API-key enrollment ${response.status}: ${await response.text()}`);
