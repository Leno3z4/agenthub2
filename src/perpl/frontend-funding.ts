...
export function encodeCollateralTransfer(token: Address, delegatedAccount: Address, amount: bigint) {
...
export async function submitCollateralTransfer(params: { walletClient: WalletClient; owner: Address; token: Address; delegatedAccount: Address; amount: bigint }) {
  return params.walletClient.writeContract({ address: params.token, abi: ERC20_TRANSFER_ABI, functionName: "transfer", args: [params.delegatedAccount, params.amount], account: params.owner });
}
