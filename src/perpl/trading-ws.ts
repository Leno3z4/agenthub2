...
const MAX_BUFFERED_MESSAGES = 250;

export interface PerplTradingCredentials {
  apiKey: string;
  privateKey: Uint8Array;
}

export interface PlaceOrderInput extends Omit<PerplOrderRequest, "mt" | "rq" | "lb" | "sn"> {
  rq?: number;
  lb?: number;
  sn?: number;
}

export interface PerplOrderResult {
  rq: number;
  sn: number;
  admission?: PerplWsMessage;
  outcome?: PerplWsMessage;
}
...
export class PerplTradingWs {
  ...
  private bufferedMessages: PerplWsMessage[] = [];
  private headBlock?: number;
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      ws.on("message", (raw) => {
        ...
        try {
          message = JSON.parse(raw.toString()) as PerplWsMessage;
        } catch {
          return;
        }
        ...
        if (this.listeners.size === 0) {
          ...
          if (this.bufferedMessages.length > MAX_BUFFERED_MESSAGES) this.bufferedMessages.shift();
          return;
        }
  ...
  onMessage(listener: (message: PerplWsMessage) => void): () => void {
    ...
    // The state cache subscribes immediately after getPerplSession() returns.
    // Replay messages received during connect/sign-in so initial account,
    // order, position and heartbeat snapshots cannot be lost in that gap.
    if (this.bufferedMessages.length > 0) {
      const buffered = this.bufferedMessages.splice(0, this.bufferedMessages.length);
  ...
  private async submitOrderUnlocked(input: PlaceOrderInput, timeoutMs: number): Promise<PerplOrderResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        unsubscribe();
        reject(new Error("Perpl order response timeout"));
      }, timeoutMs);
      const unsubscribe = this.onMessage((message) => {
        if (message.mt === 3 && message.cid === sn) {
          admission = message;
          const status = typeof message.status === "object" && message.status
            ? message.status as Record<string, unknown>
            : undefined;
          if (status?.code !== 0) {
            clearTimeout(timeout);
            unsubscribe();
            reject(new Error(typeof status?.error === "string" ? status.error : "Perpl order rejected"));
            return;
          }
          return;
        }
  ...
  async submitOrderWithRetry(input: PlaceOrderInput, timeoutMs = 8_000): Promise<PerplOrderResult> {
    ...
    try {
      return await this.submitOrder(first, timeoutMs);
    } catch (error) {
  ...
  async closePosition(params: { mkt: number; acc: number; direction: 1 | 2; size: number; leverage: number; lb: number; maxSlippageBps: number }, timeoutMs = 10_000): Promise<PerplOrderResult> {
    if (!(params.size > 0) || !(params.leverage > 0) || !(params.lb > 0) || !(params.maxSlippageBps > 0)) throw new Error("Invalid emergency close parameters");
    const t = params.direction === 1 ? 3 : 4;
    return this.submitOrderWithRetry({ mkt: params.mkt, acc: params.acc, t, p: 0, s: params.size, ms: params.maxSlippageBps, fl: 4, lv: params.leverage, lb: params.lb }, timeoutMs);
  }
