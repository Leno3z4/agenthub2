...
function state(identityId: string): State { const existing = states.get(identityId); if (existing) return existing; const created: State = { account: null, orders: new Map(), positions: new Map(), lastMessageAt: null, lastHeartbeatAt: null, headBlock: null, lastSequence: null, sequenceGap: false, ordersSnapshot: false, positionsSnapshot: false }; states.set(identityId, created); return created; }
...
export async function ensurePerplAccountState(identityId: string) {
  ...
  const unsubscribe = session.onMessage((message) => {
    ...
    if (message.mt === 19) { const account = accountFrom(message); if (account) current.account = account; current.lastSequence = numeric(message.sn); current.sequenceGap = false; return; }
    if (message.mt === 100) {
      const sequence = numeric(message.sn); const head = numeric(message.h); current.lastHeartbeatAt = Date.now(); if (head !== null) current.headBlock = head;
      if (sequence !== null && current.lastSequence !== null && sequence !== current.lastSequence + 1) { current.sequenceGap = true; current.account = null; current.orders.clear(); current.positions.clear(); current.ordersSnapshot = false; current.positionsSnapshot = false; unsubscribe(); subscriptions.delete(identityId); closePerplSession(identityId); return; }
      if (sequence !== null) current.lastSequence = sequence; return;
    }
...
function snapshotReady(current: State, staleMs: number) { return !current.sequenceGap && current.account !== null && current.ordersSnapshot && current.positionsSnapshot && current.lastHeartbeatAt !== null && Date.now() - current.lastHeartbeatAt <= staleMs && current.headBlock !== null; }
export async function getPerplState(identityId: string) { await ensurePerplAccountState(identityId); const current = state(identityId); const staleMs = Number(process.env.PERPL_STATE_STALE_MS ?? 30_000); return { subscribed: !current.sequenceGap, lastMessageAt: current.lastMessageAt, stale: !snapshotReady(current, staleMs), sequenceGap: current.sequenceGap, headBlock: current.headBlock, account: current.account, orders: [...current.orders.values()], positions: [...current.positions.values()] }; }
export async function getPerplEmergencySnapshot(identityId: string) { await ensurePerplAccountState(identityId); const current = state(identityId); const staleMs = Number(process.env.PERPL_STATE_STALE_MS ?? 30_000); if (!snapshotReady(current, staleMs)) throw new Error("Perpl account state is not fresh enough for emergency action"); return { account: current.account!, headBlock: current.headBlock!, orders: [...current.orders.values()], positions: [...current.positions.values()] }; }
