...
export interface PerplOnboardingPlan {
  ...
  amount: bigint;
  approveCalldata: Hex;
  createAccountCalldata: Hex;
}

export async function buildPerplOnboardingPlan(params: {
  ...
  decimals: number;
}): Promise<PerplOnboardingPlan> {
  const amount = parseUnits(params.amountDisplay, params.decimals);
  if (amount <= 0n) throw new Error("Collateral amount must be positive");
...
export function encodeDelegatedCall(target: Address, data: Hex): Hex {
...
export async function submitOwnerDelegatedCall(params: {
...
export async function verifyPerplAccount(params: {
  publicClient: PublicClient;
  exchange: Address;
  delegatedAccount: Address;
}) {
  const accountId = await params.publicClient.readContract({
    address: params.exchange,
    abi: PERPL_EXCHANGE_ABI,
    functionName: "getAccountByAddr",
    args: [params.delegatedAccount],
  });
