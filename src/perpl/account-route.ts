...
export async function handlePerplAccountRoute(req: IncomingMessage, res: ServerResponse, publicClient: PublicClient): Promise<boolean> {
  ...
  const limit = rateLimit(`agent-perpl-account:${clientIp(req.headers)}`, 60, 60_000); if (!limit.allowed) { json(res, 429, { error: "Too many requests" }, limit.retryAfterMs); return true; }
  const token = bearer(req); if (!token) { json(res, 401, { error: "Agent credential required" }); return true; }
  try { const credential = await authenticateAgent(token); requireAgentScope(credential, "trade:read"); const state = await getPerplAccountState({ identityId: credential.identityId, delegatedAccount: credential.delegatedAccount as Address, publicClient }); json(res, 200, state); }
  catch (error) { json(res, error instanceof Error && error.message.startsWith("Missing capability:") ? 403 : 409, { error: error instanceof Error && error.message.startsWith("Missing capability:") ? error.message : "Perpl account is not connected" }); }
  return true;
}
