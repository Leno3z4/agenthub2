...
export async function savePendingEnrollment(input: {
  id: string;
  identityId: string;
  ...
  await getDb().query(
    "INSERT INTO perpl_enrollments (id, identity_id, delegated_account, public_key, encrypted_private_key, private_key_iv, private_key_auth_tag, typed_data, mac, expires_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,to_timestamp($10 / 1000.0))",
    [input.id, input.identityId, input.delegatedAccount.toLowerCase(), input.publicKey, encrypted.ciphertext, encrypted.iv, encrypted.authTag, JSON.stringify(input.typedData), input.mac, input.expiresAt],
  );
}

export async function getPendingEnrollment(id: string, identityId: string) {
  const result = await getDb().query("SELECT * FROM perpl_enrollments WHERE id = $1 AND identity_id = $2 AND consumed_at IS NULL AND expires_at > NOW() LIMIT 1", [id, identityId]);
...
export async function consumePendingEnrollment(id: string, identityId: string) {
  const result = await getDb().query("UPDATE perpl_enrollments SET consumed_at = NOW() WHERE id = $1 AND identity_id = $2 AND consumed_at IS NULL AND expires_at > NOW()", [id, identityId]);
