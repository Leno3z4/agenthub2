...
export async function handlePerplStateRoute(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  ...
  try {
    ...
    if (message.startsWith("Missing capability:")) {
      json(res, 403, { error: message, code: "missing_capability" });
    } else if (message === "Perpl credentials are not enrolled") {
      json(res, 409, {
        error: "Perpl credentials are not enrolled for this AgentHub identity",
        code: "perpl_enrollment_required",
      });
    } else {
      console.error("Perpl state request failed", { identityId: undefined, error: message });
      json(res, 409, {
        error: "Perpl trading state is unavailable",
        code: "perpl_state_unavailable",
      });
    }
  }
  return true;
