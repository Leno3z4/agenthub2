...
function json(res: any, status: number, body: unknown, retryAfterMs = 0) { const headers: Record<string, string> = { "content-type": "application/json", "cache-control": "no-store" }; if (retryAfterMs > 0) headers["retry-after"] = String(Math.ceil(retryAfterMs / 1000)); res.writeHead(status, headers); res.end(JSON.stringify(body)); }
function limited(req: any, res: any, scope: string, max: number, windowMs: number): boolean { const result = rateLimit(`${scope}:${clientIp(req.headers)}`, max, windowMs); if (result.allowed) return false; json(res, 429, { error: "Too many requests" }, result.retryAfterMs); return true; }
async function body(req: any): Promise<Record<string, unknown>> { const chunks: Buffer[] = []; for await (const chunk of req) chunks.push(Buffer.from(chunk)); const raw = Buffer.concat(chunks); if (raw.length > 16_384) throw new Error("Request body too large"); if (!raw.length) return {}; const parsed = JSON.parse(raw.toString("utf8")); if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error("Invalid JSON body"); return parsed as Record<string, unknown>; }
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && req.url === "/api/identity/access-key") {
      if (limited(req, res, "identity-access", 3, 60_000)) return; const data = await body(req); const owner = String(data.owner ?? "") as Address;
      const message = String(data.message ?? ""); const signature = String(data.signature ?? "") as Hex;
      ...
      const delegated = await checkDelegatedAccount(owner, publicClient); if (!delegated.exists) return json(res, 409, { error: "Delegated account does not exist" });
    ...
    if (req.method === "POST" && req.url === "/api/agent/connect") {
      if (limited(req, res, "agent-connect", 10, 60_000)) return; const data = await body(req); const accessToken = String(data.identity_access_key ?? data.connection_token ?? bearer(req) ?? "");
    ...
    if (req.method === "POST" && req.url === "/api/identity/access-key/revoke") {
      if (limited(req, res, "identity-revoke", 5, 60_000)) return; const data = await body(req); const accessToken = String(data.identity_access_key ?? bearer(req) ?? "");
      if (!accessToken) return json(res, 401, { error: "Identity access key required" }); const identity = await authenticateIdentityAccessKey(accessToken); const id = String(data.access_key_id ?? "");
      if (!id || !await revokeIdentityAccessKey(id, identity.id)) return json(res, 404, { error: "Access key not found" }); return json(res, 200, { revoked: true, identity_id: identity.id });
    }
    const delegatedMatch = req.url?.match(/^\/api\/agent\/delegated-account\/(0x[a-fA-F0-9]{40})$/);
    if (req.method === "GET" && delegatedMatch) { try { return json(res, 200, await checkDelegatedAccount(delegatedMatch[1] as Address, publicClient)); } catch { return json(res, 502, { error: "Unable to check delegated account" }); } }
