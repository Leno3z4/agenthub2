...
export function DepositPanel() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled && account.delegated_account) {
          setDelegatedAccount(account.delegated_account as Address);
        }
      } catch (err) {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Unable to load deposit details");
        }
      }
    }
  ...
  async function deposit() {
    ...
    if (!delegatedAccount) {
      setError("Your delegated trading account is not available yet.");
      return;
    }

    try {
      const units = parseUnits(amount, decimals);
      if (units <= 0n) throw new Error("Enter a valid deposit amount.");
  ...
  return (
    <section className="dashboard-panel">
      ...
      <div className="balance">
        ...
        <strong>{formattedBalance.toFixed(2)} AUSD</strong>
      </div>
      ...
      <button
        ...
        disabled={isPending || !amount || !delegatedAccount}
        onClick={deposit}
      >
        {isPending ? "Confirm in wallet" : "Deposit to agent"}
      </button>
      ...
      <p className="panel-note">
        AUSD is transferred from your connected wallet to your delegated
        trading account.
      </p>
    </section>
  );
